...
export default async function AdminOutfitsPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex flex-wrap gap-2 text-sm" action="/admin/outfits" method="get">
        <input
          name="search"
        ...
        />
        <select name="status" defaultValue={sp.status ?? ""} className="rounded-xl border border-neutral-200 px-2">
          <option value="">All statuses</option>
          <option value="published">Published</option>
          <option value="draft">Draft</option>
        </select>
        <button type="submit" className="rounded-xl bg-neutral-900 px-3 py-2 text-white">
          Filter
        </button>
      </form>
